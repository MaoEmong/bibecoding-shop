...
- MVP 기준 기능(회원/상품/장바구니/주문) 중심으로 설계합니다.
...
## 인덱스 비고
- 현재 코드에서 명시된 제약/인덱스는 `UNIQUE(users.email)`, `UNIQUE(carts.buyer_id)`, `UNIQUE(cart_items(cart_id, product_id))` 중심입니다.
- 추가 인덱스는 성능 요구에 맞춰 별도 마이그레이션으로 관리하는 것을 권장합니다.

## MVP 스코프 비고
- 배송 주소/배송 상세 모델은 아직 미구현입니다.
