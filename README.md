...
이 프로젝트는 바이브 코딩 방식으로 쇼핑몰 사이트를 제작하는 것을 목표로 진행했습니다.

## 1. 프로젝트 목적과 범위

- 목적: 바이브 코딩으로 쇼핑몰 사이트 MVP 구현
- 구현 범위(현재 코드 기준):
...
- 포함 데이터:
  - 사용자 6명(기존 4명 + 로그인 가능한 테스트 계정 2명)
  - 상품, 장바구니, 주문 샘플
- 로그인 가능한 테스트 계정:
  - 구매자: `ssar@nate.com` / `123456`
  - 판매자: `cos@nate.com` / `123456`
- 참고:
  - 기존 `seller1@example.com`, `seller2@example.com`, `buyer1@example.com`, `buyer2@example.com` 계정은 `DUMMY_HASH`라 로그인 불가
